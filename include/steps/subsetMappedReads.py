        ...
        for run_id in run_ids_connections_files.keys():
            ...
            with self.declare_run(run_id) as run:
                ...
                if not self.is_option_set_in_config('Nreads'):
                    raise StepError(
                        self, "Required option 'Nreads' not set in your configuration file")
                ...
                with run.new_exec_group() as exec_group:
                    ...
                    with exec_group.add_pipeline() as pipe:
                        ...
                        # extract only reads that were aligned and include only
                        # pairs
                        if self.get_option('paired_end'):
                            samtools_view = [
                                self.get_tool(
                                    'samtools'), 'view', '-F', '0x04', '-f', '0x03', '-h',
                        ...
                        # include mate and the lines for the header of the sam
                        # format
                        ...
                        pipe.add_command(
                            dd_out, stdout_path=run.add_output_file(
                                'alignments', '%s.N%s.reads.sam' %
                                (run_id, self.get_option('Nreads')), input_paths))
