...
class Hisat2(AbstractStep):
    ...
    def __init__(self, pipeline):
        ...
        flags = ["q", "qseq", "f", "c", "ignore-quals", "nofw", "dta",
                 ...
                 "no-temp-splicesite", "no-spliced-alignment", "tmo",  "no-head", "no-sq", "omit-sec-seq", 'un-gz']
        ...
        self.add_option('rg-id', bool, default=None, optional=True, description="puts sample name in rg")

        # note to self just allow R F U and if paired in extend accordingly otherwise mixed single paired will not work  
        # Truseq is RF (R)
        self.add_option('rna-strandness', str, choices=["R", "F", "U"],   default=None, optional=False, desription="paried and are extended F-> FR, R->RF")
    ...
    def runs(self, run_ids_connections_files):
        flags = ["q", "qseq", "f", "c", "ignore-quals", "nofw", "dta",
                 "norc", "no-mixed"  ,  "no-discordant", "quiet", "qc-filter",
                 "non-deterministic", "remove-chrname", "add-chrname",
                 "no-temp-splicesite", "no-spliced-alignment", "tmo",  "no-head", "no-sq", "omit-sec-seq" ]
        ...
        if sum(res) > 1:
            raise StandardError("too many stranded flags fr, rf, ff: %s" % (res))
