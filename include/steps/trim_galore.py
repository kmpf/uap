...
# Writing to stdout and stderr
        ...
        # <run_id>_<first_read>_val_1.fq.gz
        self.add_connection('out/first_read')
        self.add_connection('out/first_read_report')  # _trimming_report.txt
        # <run_id>_<first_read>_val_1.fq.gz
        self.add_connection('out/first_read_fastqc_zip')
        # <run_id>_<first_read>_val_1.fq.html
        self.add_connection('out/first_read_fastqc_html')

        # <run_id>_<first_read>_val_2.fq.gz
        self.add_connection('out/second_read')
        self.add_connection('out/second_read_report')  # _trimming_report.txt
        # <run_id>_<second_read>_val_2.fq.gz
        self.add_connection('out/second_read_fastqc_zip')
        # <run_id>_<second_read>_val_2.fq.html
        self.add_connection('out/second_read_fastqc_html')
...
# description='If specified any output to STDOUT or STDERR will be
# suppressed.')
        self.add_option(
            'clip_R1',
            int,
    ...
    def runs(self, run_ids_connections_files):
        ...
        for run_id in run_ids_connections_files.keys():
            ...
            with self.declare_run(run_id) as run:
                ...
                read_types['first_read'] = re.sub(
                    r"^_(.*)\.?.*$", r"\g<1>", read_types['first_read'])

                if not input_paths['in/second_read'][0] is None:
                    read_types['second_read'] = re.sub(
                        r"^_(.*)\.?.*$", r"\g<1>", read_types['second_read'])

                if input_paths['in/first_read'][0].endswith('.gz'):
                    if not input_paths['in/second_read'][0] is None:
                        run.add_output_file(
                            'first_read', '%s_%s_val_1.fq.gz' %
                            (run_id, read_types['first_read']), input_paths['in/first_read'])
                    else:
                        run.add_output_file(
                            'first_read', '%s_%s_trimmed.fq.gz' %
                            (run_id, read_types['first_read']), input_paths['in/first_read'])

                else:
                    if not input_paths['in/second_read'][0] is None:
                        run.add_output_file(
                            'first_read', '%s_%s_val_1.fq' %
                            (run_id, read_types['first_read']), input_paths['in/first_read'])
                    else:
                        run.add_output_file(
                            'first_read', '%s_%s_trimmed.fq' %
                            (run_id, read_types['first_read']), input_paths['in/first_read'])

                if not input_paths['in/second_read'][0] is None:
                    run.add_output_file(
                        'first_read_fastqc_zip', '%s_%s_val_1_fastqc.zip' %
                        (run_id, read_types['first_read']), input_paths['in/first_read'])
                    run.add_output_file(
                        'first_read_fastqc_html', '%s_%s_val_1_fastqc.html' %
                        (run_id, read_types['first_read']), input_paths['in/first_read'])
                else:
                    run.add_output_file(
                        'first_read_fastqc_zip', '%s_%s_trimmed_fastqc.zip' %
                        (run_id, read_types['first_read']), input_paths['in/first_read'])
                    run.add_output_file(
                        'first_read_fastqc_html', '%s_%s_trimmed_fastqc.html' %
                        (run_id, read_types['first_read']), input_paths['in/first_read'])

                run.add_output_file(
                    'first_read_report',
                    '%s_trimming_report.txt' %
                    os.path.basename(
                        input_paths['in/first_read'][0]),
                    input_paths['in/first_read'])

                if not input_paths['in/second_read'][0] is None:
                    if input_paths['in/second_read'][0].endswith('.gz'):
                        run.add_output_file(
                            'second_read', '%s_%s_val_2.fq.gz' %
                            (run_id, read_types['second_read']), input_paths['in/second_read'])
                    else:
                        run.add_output_file(
                            'second_read', '%s_%s_val_2.fq' %
                            (run_id, read_types['second_read']), input_paths['in/second_read'])

                    run.add_output_file(
                        'second_read_fastqc_zip', '%s_%s_val_2_fastqc.zip' %
                        (run_id, read_types['second_read']), input_paths['in/second_read'])
                    run.add_output_file(
                        'second_read_fastqc_html', '%s_%s_val_2_fastqc.html' %
                        (run_id, read_types['second_read']), input_paths['in/second_read'])
                    run.add_output_file(
                        'second_read_report',
                        '%s_trimming_report.txt' %
                        os.path.basename(
                            input_paths['in/second_read'][0]),
