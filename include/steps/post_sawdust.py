...
class Post_Sawdust(AbstractStep):
    ...
    def __init__(self, pipeline):
        ...
        self.add_option(
            ...
            optional=False)
        self.add_option(
            'seq_type', str, choices=[
                'RNA', 'DNA'], optional=False)
        self.add_option(
            'read_type',
            str,
            choices=[
            ...
            with self.declare_run(run_id) as run:
                ...
                with run.new_exec_group() as exec_group:
                    with exec_group.add_pipeline() as pipe:
                        samtools_front = [
                            self.get_tool('samtools'), 'view', '-h', alignments]
                        pipe.add_command(samtools_front)

                        post_sawdust = [
                            self.get_tool('post_sawdust'),
                            '--library-type',
                            self.get_option('library_type'),
                            '--seq-type',
                            self.get_option('seq_type'),
                            '--read-type',
                            self.get_option('read_type'),
                            '--split-identifier-by',
                            self.get_option('split_ident')]
                        ...
                        samtools_end = [
                            self.get_tool('samtools'), 'view', '-Shb', '-']
