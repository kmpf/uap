...
class IOStep(abstract_step.AbstractStep):
    ...
    def __init__(self, pipeline, tool):
        ...
        # here for cosmetic purposes so that it shows up the step
        # documentation.
        ...
        for run_id, input_paths in self.get_run_ids_and_input_files_for_connection(
            ...
            with self.declare_run(run_id) as run:
                for in_path in input_paths:
                    ...
                    run.add_output_file(
                        annotation, misc.append_suffix_to_path(
                            os.path.basename(in_path), self._tool), [in_path])
                    run.new_exec_group()
                ...
                with process_pool.ProcessPool(self) as pool:
                    with pool.Pipeline(pool) as pipeline:
                        ...
                        if output_path[-3:] == '.gz':
                            ...
                            pipeline.append(cat)
                            pipeline.append(
                                tool_command, stdout_path=output_path)
