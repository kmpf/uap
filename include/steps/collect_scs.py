...
class CollectScs(AbstractStep):
    ...
    def __init__(self, pipeline):
        ...
        self.add_option(
            ...
            list,
            default=None,
            help="genome names of alignments")

        self.add_option(
            'rrna-aln-pos',
            str,
            default=None,
            help="rRNA counts position in succesive alignment",
            optional=False)
            ...
            with self.declare_run(run_id) as run:
                ...
                with run.new_exec_group() as exec_group:
                    with exec_group.add_pipeline() as pipe:
                        scs = [
                            self.get_tool('collect_scs'),
                            '--name',
                            run_id,
                            '--outfile',
                            out,
                            '--rrna-aln-pos',
                            self.get_option('rrna-aln-pos'),
                            '--library-type',
                            self.get_option('library-type'),
                            '--types']
